...
# Substitute: grf-football, coworld-grf-football, 8.
#
...
#
# env:
#   SMOKE_IMAGE                image, if not given as $1        (coworld-grf-football:ci)
#   SMOKE_SLUG                 game slug                        (grf-football)
#   SMOKE_GAME_BIN             game entrypoint                  (/bin/grf-football)
#   SMOKE_PLAYER_BIN           player entrypoint                (/bin/grf-football-player)
#   SMOKE_MANIFEST             manifest template path           (coworld_manifest_template.json)
#   SMOKE_SEATS                seat-count CROSS-CHECK           (8)
...
image="${1:-${SMOKE_IMAGE:-coworld-grf-football:ci}}"
slug="${SMOKE_SLUG:-grf-football}"
