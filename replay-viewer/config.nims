...
# use-after-free fixed in grf_football_replay.nim (emscripten_exit_with_live_runtime)
...
# grf_error_len(). Aborting keeps linear memory intact, and the page reads
# grf_stage_ptr/len afterwards to report what the runtime was doing.
switch(
  "passL",
  (&"""
  -o {distDir / "grf_replay.js"}
  ...
  -s EXPORTED_FUNCTIONS=_main,_malloc,_free,_grf_load_replay,_grf_frame,_grf_input,_grf_packet_ptr,_grf_packet_len,_grf_mismatch_tick,_grf_error_ptr,_grf_error_len,_grf_stage_ptr,_grf_stage_len
  """).replace("\n", " ")
)
